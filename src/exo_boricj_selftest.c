/*  =========================================================================
    exo_boricj_selftest.c - run selftests
...
// Tests for draft public classes:
    { "exo_boricj_server", exo_boricj_server_test },
#endif // EXO_BORICJ_BUILD_DRAFT_API
#ifdef EXO_BORICJ_BUILD_DRAFT_API
    { "private_classes", exo_boricj_private_selftest },
#endif // EXO_BORICJ_BUILD_DRAFT_API
    {0, 0}          //  Sentinel
};
...
{
    test_item_t *item;
    printf ("Running exo-boricj selftests...\n");
    for (item = all_tests; item->test; item++)
        item->test (verbose);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("exo_boricj_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running exo-boricj test '%s'...\n", test->testname);
        test->test (verbose);
    }
    else
        test_runall (verbose);
